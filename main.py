...
def ListMaterials(PrintID: int):
    ...
    con = sqlite3.connect("sqlite-latest.sqlite")
    cur=con.cursor()
    query = "SELECT iam.materialTypeID,it.typename,it.groupid,iam.quantity FROM industryactivitymaterials iam join invtypes it on iam.materialtypeid = it.typeid WHERE iam.activityid=1 and iam.typeid = " + str(PrintID)
